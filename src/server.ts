...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS chat_sessions (
    ...
    lesson_id TEXT,
    session_type TEXT DEFAULT 'coach',
...
app.post("/api/chat", rateLimiter(15, 60000), async (req: Request, res: Response) => {
  try {
    const { model, messages, options, stream, lessonId, sessionId } = req.body;
    const sanitizedOptions = sanitizeChatOptions(options);
    ...
    // Log chat history in SQLite database with session support
    try {
      let currentSessionId = sessionId;

      // If no session ID provided, create or retrieve one for this lesson
      if (!currentSessionId && lessonId) {
        const stmtCheckSession = db.prepare("SELECT id FROM chat_sessions WHERE lesson_id = ? AND session_type = 'coach' ORDER BY updated_at DESC LIMIT 1");
        const existingSession = stmtCheckSession.get(lessonId) as { id: number } | undefined;

        if (existingSession) {
          currentSessionId = existingSession.id;
        } else {
          // Create new session
          const stmtCreateSession = db.prepare("INSERT INTO chat_sessions (lesson_id, session_type, updated_at) VALUES (?, 'coach', CURRENT_TIMESTAMP)");
          const result = stmtCreateSession.run(lessonId);
      ...
      if (currentSessionId) {
        ...
        if (userMsg && userMsg.role === 'user') {
          const stmtInsertUser = db.prepare("INSERT OR REPLACE INTO chat_messages (id, session_id, role, content, correction) VALUES (?, ?, ?, ?, ?)");
          stmtInsertUser.run(
            `msg-${Date.now()}-user`,
            currentSessionId,
            userMsg.role,
        ...
        // Insert AI response
        const stmtInsertAI = db.prepare("INSERT OR REPLACE INTO chat_messages (id, session_id, role, content, correction) VALUES (?, ?, ?, ?, ?)");
        stmtInsertAI.run(
          `msg-${Date.now()}-ai`,
          currentSessionId,
          data.message.role,
        ...
        const stmtUpdateSession = db.prepare("UPDATE chat_sessions SET updated_at = CURRENT_TIMESTAMP WHERE id = ?");
        stmtUpdateSession.run(currentSessionId);
      }
    } catch (logErr) {
      console.error("Failed to log chat history:", logErr);
    }
...
// NEW API: Get chat history for a lesson/session
app.get("/api/chat/history", (req: Request, res: Response) => {
  try {
    const { lessonId, sessionId } = req.query;

    if (!lessonId) {
      return res.status(400).json({ error: "Missing lessonId parameter" });
    }

    // Find the most recent session for this lesson
    let currentSessionId = sessionId;
    if (!currentSessionId) {
      const stmtSession = db.prepare("SELECT id FROM chat_sessions WHERE lesson_id = ? AND session_type = 'coach' ORDER BY updated_at DESC LIMIT 1");
      const session = stmtSession.get(String(lessonId)) as { id: number } | undefined;
      if (session) {
    ...
    const stmtMessages = db.prepare("SELECT id, role, content, correction, created_at FROM chat_messages WHERE session_id = ? ORDER BY created_at ASC");
    const rows = stmtMessages.all(currentSessionId) as { id: string, role: string, content: string, correction: string | null, created_at: string }[];

    const messages = rows.map(row => ({
      id: row.id,
      role: row.role,
      content: row.content,
      correction: row.correction ? JSON.parse(row.correction) : null,
      timestamp: row.created_at
    }));

    res.json({ messages, sessionId: currentSessionId });
  } catch (err: any) {
    console.error("Failed to get chat history:", err);
    res.status(500).json({ error: "Failed to get chat history", details: err.message });
  }
});

// NEW API: Get lessons with theme analysis
app.get("/api/lessons/:id/details", (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const lesson = {
      ...row,
      isInitialized: Boolean(row.isInitialized),
      vocab: JSON.parse(row.vocab || "[]"),
      lines: JSON.parse(row.lines || "[]"),
      gameStartWords: JSON.parse(row.gameStartWords || "[]")
    };

    // Extract themes from script analysis
    const themes = new Set<string>();
    const mainTheme = lesson.theme || "General";
    themes.add(mainTheme);

    // Analyze lines for sub-themes
    if (lesson.lines && Array.isArray(lesson.lines)) {
      const text = lesson.lines.map((l: any) => l.text || "").join(" ").toLowerCase();

      // Common theme keywords
      const themeKeywords: Record<string, string[]> = {
        "Food & Drink": ["coffee", "tea", "food", "eat", "drink", "restaurant", "cafe", "mug", "kettle"],
        "Shopping": ["shop", "buy", "market", "store", "money", "price", "cheap", "expensive"],
        "Travel": ["travel", "trip", "journey", "train", "bus", "plane", "airport", "station"],
        "Daily Life": ["morning", "evening", "home", "house", "daily", "routine", "everyday"],
        "Nature": ["plant", "tree", "flower", "garden", "park", "nature", "green"],
        "Technology": ["phone", "computer", "internet", "online", "digital", "app"]
      };
    ...
    res.json({
      ...
      mainTheme,
      subThemes: Array.from(themes).filter(t => t !== mainTheme)
    });
  } catch (err: any) {
    console.error("Failed to get lesson details:", err);
    res.status(500).json({ error: "Failed to get lesson details", details: err.message });
  }
});
