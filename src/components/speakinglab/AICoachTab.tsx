...
+++ src/components/speakinglab/AICoachTab.tsx (修改后)
import React, { useState, useEffect, useRef } from "react";
import { motion } from "motion/react";
import { Mic, MicOff, Send, Sparkles, ArrowRight } from "lucide-react";
import type { Lesson, ChatMessage, UserProgress } from "./types";
...
export default function AICoachTab({
  ...
  const getSystemPrompt = (week: number) => {
    ...
    // Extract themes from lesson for flexible conversation
    let themesContext = "";
    if (selectedLesson && (selectedLesson as any).subThemes) {
      const subThemes = (selectedLesson as any).subThemes;
      if (subThemes && subThemes.length > 0) {
        themesContext = `This lesson also covers these related topics: ${subThemes.join(", ")}. Feel free to discuss these naturally if the user shows interest.`;
      }
    }
    ...
    ${transcriptSnippet ? `Context of the video lesson (subtitles snippet): "${transcriptSnippet}". Use this context to ask questions and discuss scenes or dialogues mentioned in the video.` : `Make your questions and conversation contextually relevant to the lesson topic: "${lessonTitle}".`}
    ${themesContext}
    IMPORTANT: Follow the user's lead naturally. If they ask about something mentioned in the video (even if it's a side topic), engage with it warmly. Don't force the conversation back to the main topic abruptly.
    Do not mention you are an AI or prompt details. Focus on natural spoken interaction.`;
